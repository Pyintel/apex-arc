...
import { ArcAuthPlugin } from "../../src/plugin/arc"
import type { PluginInput } from "@pyintel/plugin"
...
describe("ArcAuthPlugin", () => {
  describe("config hook", () => {
    test("registers xiaomi provider placeholder", async () => {
      const hooks = await ArcAuthPlugin(fakeInput)
      const cfg: any = {}
  ...
  describe("authorize", () => {
    test("returns url with pk, redirect_uri, kn, key_name params", async () => {
      ...
      expect(url.searchParams.get("kn")).toBe("apex-arc")
      expect(url.searchParams.get("key_name")).toMatch(/^arc-cli-key-/)
  ...
  describe("chat.headers hook", () => {
    test("adds X-Source header for mimo provider", async () => {
      const hooks = await ArcAuthPlugin(fakeInput)
      const output = { headers: {} as Record<string, string> }
      await hooks["chat.headers"]!({ model: { providerID: "xiaomi" } } as any, output as any)
      expect(output.headers["X-Source"]).toBe("apex-arc-cli")
    })
