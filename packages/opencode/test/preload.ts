...
process.env["ARC_TEST_TMPDIR_ROOT"] = fixtureRoot
afterAll(async () => {
...
process.env["ARC_MODELS_PATH"] = path.join(import.meta.dir, "tool", "fixtures", "models-api.json")
...
process.env["ARC_TEST_MANAGED_CONFIG_DIR"] = testManagedConfigDir
process.env["ARC_DISABLE_DEFAULT_PLUGINS"] = "true"
...
delete process.env["ARC_SERVER_PASSWORD"]
delete process.env["ARC_SERVER_USERNAME"]
delete process.env["ARC_HOME"]

// Use in-memory sqlite
process.env["ARC_DB"] = ":memory:"
...
// ARC_EXPERIMENTAL_ORCHESTRATOR; the orchestrator test suites exercise the
// feature, so enable it here (Flag is read once at import — must be set first).
process.env["ARC_EXPERIMENTAL_ORCHESTRATOR"] = "true"
