...
`,
  ...
  const meta = process.env.ARC_PLUGIN_META_FILE
  process.env.ARC_PURE = "1"
  process.env.ARC_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    if (meta === undefined) {
      delete process.env.ARC_PLUGIN_META_FILE
    } else {
      process.env.ARC_PLUGIN_META_FILE = meta
    }
  }
})
