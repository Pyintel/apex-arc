...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  if (!Flag.ARC_DISABLE_BUILTIN_SKILLS) {
    ...
    if (builtinSkillRoot && (yield* fsys.isDir(builtinSkillRoot))) {
      ...
      if (Flag.ARC_DISABLE_DOCUMENT_SKILLS) {
  ...
  if (!Flag.ARC_DISABLE_COMPOSE_SKILLS) {
  ...
  if (!Flag.ARC_DISABLE_EXTERNAL_SKILLS) {
    const externalDirs = EXTERNAL_DIRS.filter((dir) => {
      if (dir === ".claude" && Flag.ARC_DISABLE_CLAUDE_CODE_SKILLS) return false
      if (dir === ".codex" && Flag.ARC_DISABLE_CODEX_SKILLS) return false
      if (dir === ".opencode" && Flag.ARC_DISABLE_OPENCODE_SKILLS) return false
      return true
    })
