...
Config is JSON or JSONC. Arc discovers it by walking up from the cwd to the worktree root, then falls back to global.

- Project: `.arc/arc.json` or `.arc/arc.jsonc`
- Global: `~/.config/arc/arc.json` (XDG config dir)
- Extra config dirs are also searched via `$ARC_CONFIG_DIR`.

Project config merges **over** global. Always include `"$schema": "https://arc.apex-arc.com/arc/config.json"` for validation.
...
|------|------------------|-------|
| data | `~/.local/share/arc/` | memory, logs, `builtin_skills/<version>/`, bin |
| config | `~/.config/arc/` | global `arc.json` |
| cache | `~/.cache/arc/` | caches, downloaded bins |
| state | `~/.local/state/arc/` | runtime state |

Memory files live under `~/.local/share/arc/memory/`:
...
- `ARC_PURE` — run without external plugins (same as `arc --pure`). Does **not** change models or Claude Code inheritance.
- `ARC_ONLY` — pure-Arc mode: don't inherit Claude Code settings (CLAUDE.md, `~/.claude/skills`), don't read provider API keys from env, fall back to the arc-auto model.
...
```jsonc
{
  ...
  "model_groups": {
    ...
    "standard": {
      ...
      "models": ["anthropic/claude-sonnet", "openrouter/arc/arc-v2.5"]
    },
...
- A ref without `/` is a group name. If configured, Arc is **provider-aware**: it prefers a member on the caller's current provider, otherwise falls back to the group's `default`.
...
Prefer a markdown file (`.arc/agent/<name>.md`, body = system prompt) for defining a custom agent/mode — see the "Custom agents & modes" section in @guide.md. Use the `agent` config key for short, inline per-agent overrides.
...
| `voice.asr_model` | ASR model (default `xiaomi/arc-v2.5-asr`) |
| `voice.control_model` | Voice control model (default `arc/arc-v2.5`) |
