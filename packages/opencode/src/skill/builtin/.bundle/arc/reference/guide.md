...
1. **Sign in** — `arc account login <url>` runs a device flow: it prints a URL + code and opens your browser. `/connect` does the same from inside the TUI (e.g. to add OpenRouter). Other account subcommands: `logout`, `switch`, `orgs`, `open`, `console`.
2. **Pick a model** — set `"model": "provider/model"` in config, or switch live in the TUI model dialog. Provider API keys are auto-detected from environment variables (unless `ARC_ONLY=1`).
3. **List what's available** — `arc models`, `arc providers`.

## Memory: making Arc remember
...
Drop a markdown file at `.arc/command/<name>.md` (or `.arc/commands/`, `.claude/command(s)/` are also read). The frontmatter configures it; the body is the prompt template.
...
A "mode" is just a **primary agent** with its own system prompt. To give Arc a custom mode (e.g. a `general` chat mode alongside the coding-focused `build`), drop a markdown file — no code, no server changes. The frontmatter is config; the **markdown body becomes the agent's system prompt**.
...
| Path | Scope |
|------|-------|
| `.arc/agent/<name>.md` (or `agents/`) | project agent — most common |
| `.arc/mode/<name>.md` (or `modes/`) | project mode — same as an agent forced to `mode: primary` |
| `~/.config/arc/agent/<name>.md` | global agent, available in every project |
...
Verify a file loaded with `arc agent list` — your agent shows up with its `(primary)` / `(subagent)` mode.
...
Inspect/manage with `arc mcp`. Request timeout defaults to 5000ms (`timeout` per server, or `experimental.mcp_timeout` globally).
...
- **Durable** jobs persist to `<project>/.arc/scheduled_tasks.json` and survive restart; non-durable jobs live only for the session.
...
To add tools, hooks, or skills, use the `self-extend` skill — it covers writing `.arc/tools/*.ts`, `.arc/hooks/*.ts`, and `.arc/skills/*/SKILL.md`, all hot-reloaded on the next turn.
