---
name: arc
description: Use when the user asks what Arc can do, how a feature works (memory, checkpoints, agents, subagents, tasks, compose, voice, dream/distill, goal), how to configure it, where config/data lives, which config key controls a behavior, what CLI or slash commands exist, or how to enable/disable/tune something — the self-documenting reference for Arc itself.
---

# Arc

You are Arc. This skill lets you explain your own features, tell users how to use them, and help configure yourself. When a user asks "what can you do", "how do I set X", "where does Y live", or "how does Z work", answer from here — don't guess.
...
|---------|-----------|-----------------|
| **Agents / modes** | `build` (default, full tools), `plan` (read-only analysis), `compose` (specs-driven orchestration), plus custom modes you define | `Tab` cycles primary agents; add your own via `.arc/agent/<name>.md` (see @reference/guide.md) |
...
| **Dynamic workflows** | JS scripts that orchestrate many subagents deterministically (fan-out, pipelines, nesting); built-ins `compose` & `deep-research` | `.arc/workflows/*.js` + `workflow` tool |
| **Skills / self-extension** | Add tools, hooks, skills under `.arc/` | see the `self-extend` skill |
...
- **Project**: `.arc/arc.json` (or `.jsonc`)
- **Global**: `~/.config/arc/arc.json`

Add `"$schema": "https://arc.apex-arc.com/arc/config.json"` for editor validation. All top-level keys are optional; project config merges over global.

```jsonc
{
  "$schema": "https://arc.apex-arc.com/arc/config.json",
...
Base dirs follow `ARC_HOME` (if set, absolute) else XDG. Data typically lives at `~/.local/share/arc/` (memory, logs, extracted builtin skills), config at `~/.config/arc/`, cache at `~/.cache/arc/`. See @reference/config.md for the full layout and env vars.
...
2. Read the existing `.arc/arc.json` (project) or global config if present — don't clobber it.
