...
const PACKAGE_NAME = "@apex-arc/cli"
...
export class Service extends Context.Service<Service, Interface>()("@apex-arc/Installation") {}
    ...
    Effect.gen(function* () {
      ...
      const upgradeCurl = Effect.fnUntraced(
        function* (target: string) {
          ...
          const response = yield* httpOk.execute(HttpClientRequest.get(process.env.ARC_INSTALL_SCRIPT_URL ?? "https://apex.pyintel.cc/arc/install"))
      ...
      const upgradeCurlWindows = Effect.fnUntraced(function* (target: string) {
        ...
        const stageDir = path.join(os.tmpdir(), `arc_upgrade_${pid}`)

        // Download new version to staging dir (reuses install.ps1 logic)
        const installScriptUrl = process.env.ARC_INSTALL_SCRIPT_URL ?? "https://apex.pyintel.cc/arc/install.ps1"
        const downloadResult = yield* run(
          ...
          { env: { ARC_INSTALL_DIR: stageDir, VERSION: target, INSTALL_SCRIPT_URL: installScriptUrl } },
        )
        if (downloadResult.code !== 0) return downloadResult

        // Replace in-place: Windows allows renaming a running exe
        const stagedExe = path.join(stageDir, "apex-arc.exe")
      ...
      const methodImpl = Effect.fn("Installation.method")(function* () {
        if (process.execPath.includes(path.join(".arc", "bin"))) return "curl" as Method
      ...
      const latestImpl = Effect.fn("Installation.latest")(function* (installMethod?: Method) {
        ...
        if (detectedMethod === "curl") {
          ...
          // ARC_FDS_BASE to mirror the install script.
          const base = (process.env.ARC_FDS_BASE || "https://apex.pyintel.cc/arc/releases").replace(
            /\/+$/,
            "",
          )
