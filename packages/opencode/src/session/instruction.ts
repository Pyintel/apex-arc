...
const FILES = [
  "AGENTS.md",
  ...(Flag.ARC_DISABLE_CLAUDE_CODE_PROMPT ? [] : ["CLAUDE.md"]),
...
function globalFiles() {
  ...
  if (Flag.ARC_CONFIG_DIR) {
    files.push(path.join(Flag.ARC_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.ARC_DISABLE_CLAUDE_CODE_PROMPT) {
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const relative = Effect.fnUntraced(function* (instruction: string) {
        ...
        return yield* fs
          .globUp(instruction, Flag.ARC_CONFIG_DIR, Flag.ARC_CONFIG_DIR)
          .pipe(Effect.catch(() => Effect.succeed([] as string[])))
      })
