  ...
  Effect.gen(function* () {
    ...
    return Service.of({
      environment: Effect.fn("SystemPrompt.environment")(function* (model: Provider.Model, now: number) {
        ...
        const base = [
          [
            `You are Pyintel Helix. You are an interactive agent that helps users with software engineering tasks. Use the instructions below and the tools available to you to assist the user. Unless the user has explicitly restricted permissions, you have access to the full runtime toolset and should use it when helpful.`,
