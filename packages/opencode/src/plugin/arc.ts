...
const log = Log.create({ service: "plugin.arc" })

const PLATFORM_URL = process.env.MIMO_PLATFORM_URL || "https://platform.apex-arc.com"

function getKeyName(): string {
  const filePath = path.join(Global.Path.data, "arc-key-name")
  try {
    const existing = fs.readFileSync(filePath, "utf-8").trim()
    if (existing) return existing
  } catch {}
  const name = `arc-cli-key-${crypto.randomBytes(4).toString("hex")}`
...
export async function ArcAuthPlugin(_input: PluginInput): Promise<Hooks> {
  return {
    config: async (input) => {
      input.provider ??= {}
      ...
      // api: https://api.apex-arc.com/v1) — hardcoding "Arc" here collided with
      // the free "arc" provider's display name and confused users.
      ...
      methods: [
        {
          ...
          authorize: async () => {
            ...
            log.info("arc oauth server started", { port })
            ...
            const serverCallbackPromise = new Promise<{ sk?: string; uid: string; url?: string }>((resolve, reject) => {
              ...
              server.on("request", (req, res) => {
                ...
                log.info("arc oauth callback received", { path: url.pathname, query: url.search.substring(0, 100) })

                const u = url.searchParams.get("u")

                if (!u) {
                  log.warn("arc oauth callback missing u param")
                ...
                try {
                  ...
                  log.info("arc oauth decrypt success", { uid: result.uid, url: result.url })
                  res.writeHead(302, { Location: `${PLATFORM_URL}/authorize/callback?status=success` })
                  res.end()
                  clearTimeout(timeout)
                  resolve(result)
                } catch (err) {
                  log.error("arc oauth decrypt failed", { error: err })
    ...
    "chat.headers": async (input, output) => {
      ...
      output.headers["X-Source"] = "apex-arc-cli"
    },
  }
}
