...
import { createOpencodeClient } from "@pyintel/sdk"
import { Flag } from "../flag/flag"
import { HelixOnlyPlugin } from "./helix"
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Plugin.state")(function* (ctx) {
        ...
        const client = createOpencodeClient({
          ...
          headers: Flag.ARC_SERVER_PASSWORD
            ? {
                Authorization: `Basic ${Buffer.from(`${Flag.ARC_SERVER_USERNAME ?? "arc"}:${Flag.ARC_SERVER_PASSWORD}`).toString("base64")}`,
              }
            : undefined,
        ...
        const plugins = Flag.ARC_PURE ? [] : (cfg.plugin_origins ?? [])
        if (Flag.ARC_PURE && cfg.plugin_origins?.length) {
