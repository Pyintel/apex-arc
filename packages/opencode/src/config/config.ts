...
import { EffectFlock } from "@pyintel/shared/util/effect-flock"
...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in arc config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
const InfoSchema = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for apex-arc serve and web commands",
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://apex.pyintel.cc/arc/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://apex.pyintel.cc/arc/docs/agents" }),
...
export class Service extends Context.Service<Service, Interface>()("@apex-arc/Config") {}

function globalConfigFile() {
  const candidates = ["arc.jsonc", "arc.json", "config.json"].map((file) =>
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema || data.$schema === "https://opencode.ai/config.json") {
        data.$schema = "https://apex.pyintel.cc/arc/config.json"
        const edits = modify(text, ["$schema"], "https://apex.pyintel.cc/arc/config.json", {
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      let result: Info = pipe(
        ...
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "arc.json"))),
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "arc.jsonc"))),
      )
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://apex.pyintel.cc/arc/config.json"
      ...
      const globalConfigFile = path.join(Global.Path.config, "arc.jsonc")
      if (
        !existsSync(path.join(Global.Path.config, "config.json")) &&
        !existsSync(path.join(Global.Path.config, "arc.json")) &&
        !existsSync(globalConfigFile)
      ) {
        const starter = '{\n  "$schema": "https://apex.pyintel.cc/arc/config.json"\n}\n'
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "ARC_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            log.debug("fetching remote config", { url: `${url}/.well-known/apex-arc` })
            const response = yield* Effect.promise(() =>
              fetch(`${url}/.well-known/apex-arc`, { signal: AbortSignal.timeout(1000) }),
            )
            if (!response.ok) {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://apex.pyintel.cc/arc/config.json"
            const source = `${url}/.well-known/apex-arc`
        ...
        if (Flag.ARC_CONFIG) {
          yield* merge(Flag.ARC_CONFIG, yield* loadFile(Flag.ARC_CONFIG))
          log.debug("loaded custom config", { path: Flag.ARC_CONFIG })
        }

        if (!Flag.ARC_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("arc", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.ARC_CONFIG_DIR) {
          log.debug("loading config from ARC_CONFIG_DIR", { path: Flag.ARC_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []

        // Load Claude Code commands first so .arc commands override on name collision.
        ...
        for (const dir of directories) {
          if (dir.endsWith(".arc") || dir === Flag.ARC_CONFIG_DIR) {
            for (const file of ["arc.json", "arc.jsonc"]) {
          ...
          // Auto-discovered plugins under `.arc/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.ARC_CONFIG_CONTENT) {
          const source = "ARC_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.ARC_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from ARC_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["ARC_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("ARC_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (!Flag.ARC_DISABLE_CLAUDE_CODE_MCP) {
        ...
        if (Flag.ARC_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.ARC_PERMISSION))
        }
        ...
        if (Flag.ARC_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.ARC_DISABLE_PRUNE) {
