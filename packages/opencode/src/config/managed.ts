...
const MANAGED_PLIST_DOMAIN = "ai.apex-arc.managed"
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/apex-arc"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "apex-arc")
    default:
      return "/etc/apex-arc"
  }
}
...
export function managedConfigDir() {
  return process.env.ARC_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
