...
export const dict = {
  ...
  "tui.home.agreement.prefix": "Используя ARC, вы соглашаетесь с нашими ",
  ...
  "tui.tips.share":
    "Выполните {highlight}/share{/highlight}, чтобы получить публичную ссылку на диалог на apex.pyintel.cc/arc",
  ...
  "tui.tips.config_files":
    "Создайте {highlight}arc.json{/highlight} для серверных настроек и {highlight}tui.json{/highlight} для настроек TUI",
  "tui.tips.global_config":
    "Поместите настройки TUI в {highlight}~/.config/apex-arc/tui.json{/highlight} как глобальные",
  "tui.tips.schema":
  ...
  "tui.tips.mcp_oauth":
    "ARC автоматически обрабатывает OAuth для удалённых MCP-серверов с авторизацией",
  "tui.tips.custom_command":
    "Добавляйте файлы {highlight}.md{/highlight} в {highlight}.arc/command/{/highlight} для повторно используемых пользовательских команд",
  ...
  "tui.tips.custom_agent":
    "Добавляйте файлы {highlight}.md{/highlight} в {highlight}.arc/agent/{/highlight} для специализированных AI-персон",
  ...
  "tui.tips.formatter":
    "ARC автоматически форматирует файлы с помощью prettier, gofmt, ruff и других инструментов",
  ...
  "tui.tips.lsp": "ARC использует LSP-серверы для интеллектуального анализа кода",
  "tui.tips.custom_tool":
    "Создавайте файлы {highlight}.ts{/highlight} в {highlight}.arc/tools/{/highlight}, чтобы определить новые LLM-инструменты",
  "tui.tips.tool_scripts":
    "Определения инструментов могут вызывать скрипты на Python, Go и других языках",
  "tui.tips.plugins":
    "Добавляйте файлы {highlight}.ts{/highlight} в {highlight}.arc/plugin/{/highlight} для подключения хуков событий",
  ...
  "tui.tips.plugin_protect":
    "Создайте плагин, который запретит ARC читать чувствительные файлы",
  "tui.tips.run":
    "Используйте {highlight}apex-arc run{/highlight} для неинтерактивных скриптов",
  "tui.tips.continue":
    "Используйте {highlight}apex-arc --continue{/highlight}, чтобы продолжить последнюю сессию",
  "tui.tips.attach_cli":
    "Используйте {highlight}apex-arc run -f file.ts{/highlight}, чтобы прикрепить файлы из командной строки",
  "tui.tips.format_json":
    "Используйте {highlight}--format json{/highlight} для машинно-читаемого вывода в скриптах",
  "tui.tips.serve":
    "Запустите {highlight}apex-arc serve{/highlight} для headless-доступа к API ARC",
  "tui.tips.attach_server":
    "Используйте {highlight}apex-arc run --attach{/highlight}, чтобы подключиться к запущенному серверу",
  "tui.tips.upgrade":
    "Выполните {highlight}apex-arc upgrade{/highlight}, чтобы обновиться до последней версии",
  "tui.tips.auth_list":
    "Выполните {highlight}apex-arc auth list{/highlight}, чтобы увидеть всех настроенных провайдеров",
  "tui.tips.agent_create":
    "Выполните {highlight}apex-arc agent create{/highlight}, чтобы создать агента в пошаговом режиме",
  "tui.tips.github_trigger":
    "Используйте {highlight}/apex-arc{/highlight} в issue/PR на GitHub, чтобы запускать AI-действия",
  "tui.tips.github_install":
    "Выполните {highlight}apex-arc github install{/highlight}, чтобы настроить GitHub-workflow",
  "tui.tips.github_oc":
  ...
  "tui.tips.theme_files":
    "Создавайте JSON-файлы тем в каталоге {highlight}.arc/themes/{/highlight}",
  ...
  "tui.tips.debug_config":
    "Выполните {highlight}apex-arc debug config{/highlight}, чтобы диагностировать проблемы конфигурации",
  "tui.tips.print_logs":
  ...
  "tui.model.apex-arc_auto.name": "Arc Auto (Arc-V2.5, временно бесплатно)",
  ...
  "tui.dialog.token_plan.link": "Arc Token Plan",
  ...
  "tui.toast.update_available.success": "Обновлено до ARC v{{version}}. Пожалуйста, перезапустите приложение.",
  ...
  "tui.voice.error.no_auth": "Используйте /connect для подключения к Arc или настройте voice.asr_model для другого провайдера",
  ...
  // Arc Auto (free) — TUI login dialog
  "tui.dialog.login.apex-arc_free": "Arc Auto (free)",
  "tui.dialog.login.apex-arc_free.desc": "Анонимный бесплатный канал — вход не требуется",
  "tui.dialog.login.apex-arc_free.success": "Arc Auto (free) готов — модель по умолчанию: apex-arc/apex-arc-auto",
  "tui.dialog.login.apex-arc_free.unavailable": "Провайдер Arc Auto (free) не загружен",
  "tui.dialog.login.flow.title": "Вход в Arc",
  ...
  "cli.providers.apex-arc.recommended_hint": "рекомендуется",
  "cli.providers.apex-arc_free.hint": "Анонимный бесплатный канал / apex-arc-auto",
  "cli.providers.apex-arc_free.verifying": "Проверка канала Arc Auto (free)...",
  "cli.providers.apex-arc_free.ready": "Канал Arc Auto (free) готов",
  "cli.providers.apex-arc_free.failed": "Не удалось проверить Arc Auto (free)",
  "cli.providers.apex-arc_free.default_set": "Модель по умолчанию: apex-arc/apex-arc-auto (контекст 1M, бесплатно)",
  "cli.providers.apex-arc_free.usage_hint":
    "Вход не требуется — просто запустите apex-arc. Для платных/премиум-моделей выберите вход через браузер Arc.",
  "cli.providers.apex-arc_login.decrypt_retry": "Ошибка расшифровки, повторите попытку (осталось попыток: {remaining})",
  "cli.providers.apex-arc_login.decrypt_exhausted": "Ошибка расшифровки, превышено максимальное число попыток",
  ...
  "trust.capabilities": "ARC сможет читать, редактировать и выполнять файлы здесь.",
