...
export const dict = {
  ...
  "tui.home.agreement.prefix": "ARC をご利用いただくことで、",
  ...
  "tui.tips.config_files":
    "サーバ設定は {highlight}arc.json{/highlight}、TUI 設定は {highlight}tui.json{/highlight} を作成します",
  "tui.tips.global_config":
    "TUI 設定をグローバルに置くなら {highlight}~/.config/apex-arc/tui.json{/highlight} を使います",
  ...
  "tui.tips.mcp_oauth": "ARC は認証が必要なリモート MCP サーバの OAuth を自動処理します",
  "tui.tips.custom_command":
    "{highlight}.arc/command/{/highlight} に {highlight}.md{/highlight} ファイルを追加すると再利用可能なカスタムプロンプトを定義できます",
  ...
  "tui.tips.custom_agent":
    "{highlight}.arc/agent/{/highlight} に {highlight}.md{/highlight} ファイルを追加して専用 AI ペルソナを作れます",
  ...
  "tui.tips.formatter": "ARC は prettier、gofmt、ruff などでファイルを自動整形します",
  ...
  "tui.tips.lsp": "ARC はインテリジェントなコード解析に LSP サーバを使います",
  "tui.tips.custom_tool":
    "{highlight}.arc/tools/{/highlight} に {highlight}.ts{/highlight} ファイルを作って新しい LLM ツールを定義します",
  "tui.tips.tool_scripts": "ツール定義から Python、Go などのスクリプトを呼び出せます",
  "tui.tips.plugins":
    "{highlight}.arc/plugin/{/highlight} に {highlight}.ts{/highlight} ファイルを追加してイベントフックを実装できます",
  "tui.tips.plugin_notify": "プラグインでセッション完了時に OS 通知を送れます",
  "tui.tips.plugin_protect": "プラグインを書いて ARC が機密ファイルを読まないようにできます",
  "tui.tips.run": "{highlight}apex-arc run{/highlight} で非対話的なスクリプト実行ができます",
  "tui.tips.continue": "{highlight}apex-arc --continue{/highlight} で前回のセッションを再開します",
  "tui.tips.attach_cli": "{highlight}apex-arc run -f file.ts{/highlight} で CLI からファイルを添付できます",
  "tui.tips.format_json": "{highlight}--format json{/highlight} を使うとスクリプトで機械可読な出力を得られます",
  "tui.tips.serve": "{highlight}arc serve{/highlight} を実行するとヘッドレスで ARC API にアクセスできます",
  "tui.tips.attach_server": "{highlight}apex-arc run --attach{/highlight} で起動中のサーバに接続します",
  "tui.tips.upgrade": "{highlight}arc upgrade{/highlight} で最新バージョンに更新します",
  "tui.tips.auth_list": "{highlight}arc auth list{/highlight} で設定済みプロバイダを確認します",
  "tui.tips.agent_create": "{highlight}arc agent create{/highlight} でガイド付きエージェント作成を行います",
  "tui.tips.github_trigger": "GitHub の issue/PR で {highlight}/arc{/highlight} を入力すると AI を起動します",
  "tui.tips.github_install": "{highlight}arc github install{/highlight} で GitHub workflow を設定します",
  ...
  "tui.tips.theme_files": "{highlight}.arc/themes/{/highlight} ディレクトリに JSON テーマファイルを作成します",
  ...
  "tui.tips.debug_config": "{highlight}arc debug config{/highlight} で設定の問題を調査します",
  ...
  "tui.model.arc_auto.name": "Arc Auto（Arc-V2.5 期間限定無料）",
  ...
  "tui.toast.update_available.success": "ARC v{{version}} に更新しました。アプリケーションを再起動してください。",
  ...
  "tui.voice.error.no_auth": "/connect で Arc アカウントに接続するか、voice.asr_model で別のプロバイダーを設定してください",
  ...
  "tui.dialog.login.arc_free.desc": "ログイン不要の匿名無料チャネル",
  "tui.dialog.login.arc_free.success": "Arc Auto (free) の準備完了 — デフォルトモデルを arc/arc-auto に設定しました",
  "tui.dialog.login.arc_free.unavailable": "Arc Auto (free) プロバイダーが読み込まれていません",
  "tui.dialog.login.flow.title": "Arc ログイン",
  ...
  "cli.providers.arc.recommended_hint": "推奨",
  "cli.providers.arc_free.hint": "匿名無料チャネル / arc-auto",
  "cli.providers.arc_free.verifying": "Arc Auto (free) チャネルを検証中...",
  "cli.providers.arc_free.ready": "Arc Auto (free) チャネル準備完了",
  "cli.providers.arc_free.failed": "Arc Auto (free) の自己診断に失敗",
  "cli.providers.arc_free.default_set": "デフォルトモデルを arc/arc-auto に設定（1M コンテキスト、無料）",
  "cli.providers.arc_free.usage_hint":
    "ログイン不要 — そのまま arc を実行できます。有料/上位モデルを利用する場合は Arc ブラウザログインを選択してください。",
  "cli.providers.arc_login.decrypt_retry": "復号に失敗しました、再試行してください（残り {remaining} 回）",
  "cli.providers.arc_login.decrypt_exhausted": "復号に失敗しました、最大再試行回数に達しました",
  ...
  "trust.capabilities": "ARC はここでファイルの読み取り、編集、実行が可能になります。",
