...
import arc from "./theme/arc.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@pyintel/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light"; plain?: boolean }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = props.plain ? "system" : config.theme ?? kv.get("theme", "arc")
        draft.active = typeof active === "string" ? active : "arc"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.arc, store.mode)
    })
