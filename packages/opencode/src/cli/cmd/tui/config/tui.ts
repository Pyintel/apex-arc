...
export class Service extends Context.Service<Service, Interface>()("@apex-arc/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  // Every config dir we may read from: global config dir, any `.arc`
  // folders between cwd and home, and ARC_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.ARC_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 4. `.arc` directories (and ARC_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".arc") || dir === Flag.ARC_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".arc") && dir !== Flag.ARC_CONFIG_DIR) continue
...
const TUI_SCHEMA_URL = "https://apex.pyintel.cc/arc/tui.json"
...
async function load(text: string, configFilepath: string): Promise<Info> {
  return ConfigVariable.substitute({ text, type: "path", path: configFilepath, missing: "empty" })
    ...
    .then((data) => {
      ...
      // (mirroring the old arc.json shape) still get their settings applied.
