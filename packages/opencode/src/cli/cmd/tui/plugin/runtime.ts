...
import { Flock } from "@pyintel/shared/util/flock"
import { Flag } from "@/flag/flag"
import { Glob } from "@pyintel/shared/util/glob"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".arc"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".arc", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".arc", "tui.json"),
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Info }) {
  ...
  try {
    await Instance.provide({
      directory: cwd,
      fn: async () => {
        const records = Flag.ARC_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.ARC_PURE && config.plugin_origins?.length) {
        ...
        // File-based TUI plugins from .arc/tui/*.{ts,tsx}
        const configDirs = [
          path.join(Global.Path.config, "tui"),
          path.join(cwd, ".arc", "tui"),
        ]
