...
/**
 * Resolve apex-arc's four base directories (config/data/state/cache)
 * from environment variables.
 *
 * If ARC_HOME is set and non-empty, the four paths are subdirectories
...
export function resolveApexArcHome(env: NodeJS.ProcessEnv = process.env): ResolvedPaths {
  ...
  if (home) {
    if (!path.isAbsolute(home)) {
      throw new Error(
        `ARC_HOME must be an absolute path, got: ${JSON.stringify(home)}`,
      )
    }
    return {
      mode: "apex_arc_home",
...
export namespace Global {
  export class Service extends Context.Service<Service, Interface>()("@apex-arc/Global") {}
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const { data, cache, config, state } = yield* Effect.sync(() => resolveApexArcHome())
